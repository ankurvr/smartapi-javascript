...
var SmartApi = function (params) {
    ...
    self.feed_token = params.feed_token || null;
    self.jwt_token = params.jwt_token || null;
    ...
    address(function (err, addrs) {
        ...
        self.mac_addr = addrs !== undefined ? addrs.mac : "fe80::216e:6507:4b90:3719";
    });
    ...
    requestInstance.interceptors.response.use(response => {
        ...
        if (response.status === 200) {
            if (response.data.success || response.data.status) {
                ...
                if (response.data.errorCode === "AG8001" && self.session_expiry_hook !== null) {
     ...
     */
    self.setFeedToken = function(feed_token) {
     ...
     */
    self.setJWTToken = function(jwt_token) {
        ...
        token_data
            .then(response => {
                if (response.status) {
                    ...
                    self.setFeedToken(response.data.feedToken);
                    self.setJWTToken(response.data.jwtToken);
                }
            }).catch(function (err) {
                throw err
            });
    ...
    self.generateToken = function (refresh_token) {
        let token_data = post_request("generate_token", { "refreshToken": refresh_token });
    ...
    self.ruleList = function (params) {
        ...
        if (Array.isArray(params.status))
            return post_request("rule_list", params);
        else
            return {
                status: 500,
                message: "Invalid status. Please refer Smart API documentation for more details.",
            };
     ...
     * @param {string} tradingsymbol
     */
    self.getLtpData = function(params) {
        return post_request("ltp_data", params);
    }
    ...
    function post_request(route, params, responseType, responseTransformer) {
        return request_util(route, "POST", params || {}, responseType, responseTransformer);
    }
